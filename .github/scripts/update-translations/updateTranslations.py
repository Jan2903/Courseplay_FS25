        ...
        if os.path.isfile(f):
            ...
            try:
                ...
                for entry in subRoot.iter('text'):
                    ...
                    lang_text = entry.attrib.get('text', '')
                    base_en = entry.attrib.get('base_en')
                    
                    for item in categories.values(): 
                        if name in item:
                            master_en = item[name].get('en', '')
                            
                            # Determine if this entry is an untranslated English fallback
                            is_untranslated = False
                            if base_en is not None:
                                # If it has the tracking tag, it's untranslated if the text hasn't been edited
                                if lang_text == base_en:
                                    is_untranslated = True
                            else:
                                # Legacy Check: For old files before we added the base_en tag.
                                # If the text perfectly matches the current master English, assume it's untranslated.
                                if language != "en" and lang_text == master_en:
                                    is_untranslated = True

                            # Only keep the translation if it has actually been translated
                            if not is_untranslated:
                                if not language in item[name]:
                                    item[name][language] = lang_text
    ...
    for index, curLanguage in enumerate(supportedLanguages):
        ...
        for category, entry in categories.items():
            ...
            for name, data in entry.items():
                ...
                # Check if we are applying a fallback
                is_fallback = False
                ...
                # Tag it as a fallback so we can safely overwrite it next time English updates
                if is_fallback and curLanguage != "en":
                    e.set("base_en", enText)
